  ...
  <div class="page">
    <div class="page-box">
      <van-pull-refresh v-model="loadings" @refresh="onRefresh" loading-text="加载中">
        <van-list v-if="list.length !== 0" class="list-box" v-model:loading="loading" :finished="finished" :finished-text="text" @load="onLoad">
          <div class="box" v-for="item in list" :key="item.id">
            ...
            <div class="box-content flex">
              ...
              <div class="right">
                ...
                <div class="data">转赠时间：{{item.createTime}}</div>
                <div class="data">转赠钱包：{{item.toAddress}}</div>
              </div>
            </div>
          </div>
        </van-list>
