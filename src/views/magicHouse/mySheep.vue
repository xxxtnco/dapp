  ...
  <div class="page">
    <div class="page-box">
      ...
      <div class="header flex-sb">
        <div @click="changeType(1)" :class="active === 1 ? 'active':''">全部({{info.total}}只)</div>
        <div @click="changeType(2)" :class="active === 2 ? 'active':''">牧羊场({{info.sheepwalkNums}}只)</div>
        <div @click="changeType(3)" :class="active === 3 ? 'active':''">暂停产出({{info.stopNums}}只)</div>
      </div>
...
async function getList() {
  ...
  info.value = data;
  list.value = [...list.value, ...data.sheepList.records];
  total.value = data.total;
  if (active.value === 1) {
    total.value = data.total;
  } else if (active.value === 2) {
    total.value = data.sheepwalkNums;
  }else{
    total.value = data.stopNums;
  }
}
